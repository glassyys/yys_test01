## python3 sql_est_008_f02.py /home/p190872/chksrc/SIDHUB/sid/msid_cntr_d_rgn_s_01.sql
## 파이썬으로 쿼리추출 소스코드 요청
## 1.추출 대상 확장자
## .sh
## .hql
...
## 저장 위치는 기존과 동일:
## 현재실행디렉토리/out/
## "dual"은 제외하지만 쿼리내부에 "dualsim"이 있는 경우는 추출하도록 원본소스 로직유지하고 수정한전체소스
## "dual" 은 제외
## 하지만 "dualsim", "mydual_table" 처럼 단어 일부로 포함된 경우는 허용
## " dual ", "from dual" 처럼 독립 단어 dual 만 제외
...
# dual 단어만 제외 (dualsim 허용)
DUAL_EXCLUDE_PATTERN = re.compile(r'\bdual\b', re.IGNORECASE)
    ...
    try:
        ...
        while pos < length:
            ...
            if query:
                ...
                # dual 단어만 제외 (dualsim 허용)
                if DUAL_EXCLUDE_PATTERN.search(lower_query):
                    pos = end
                    continue
