## python3 sql_est_009_f01.py /home/p190872/chksrc/SIDHUB/sid/msid_cntr_d_rgn_s_01.sql
## 파이썬으로 쿼리추출 소스코드 요청
## 1.추출 대상 확장자
## .sh
## .hql
...
## 기존로직 유지하고 아래 내용 반영하여 수정한 전체소스
## 
## 기존로직 유지하고 "절대경로포함소스디렉토리" 대신 "절대경로포함소스디렉토리와파일명"을 파라미터로 주면
## 파일기준으로 쿼리추출하고 파일명.dat로 생성하도록 로직 변경한 전체소스

#!/usr/bin/env python3
# 실행방법:
# python3 프로그램명.py 절대경로포함소스디렉토리와파일명
...
if len(sys.argv) != 2:
    print("사용법: python3 프로그램명.py 절대경로포함소스디렉토리와파일명")
    sys.exit(1)
...
def main():
    ...
    output_filename = f"{input_name_only}.dat"
