## python3 sql_lng_002.py /home/p190872/chksrc/test
## python3 sql_lng_002.py /home/p190872/chksrc/SIDHUB
## python3 sql_lng_002.py /NAS/MIDP/DBMSVC/MIDP/SID/SRC/SIDHUB
## python3 sql_lng_002.py /NAS/MIDP/DBMSVC/MIDP/SID
## python3 sql_lng_002.py /NAS/MIDP/DBMSVC/MIDP/TMT
## python3 sql_lng_002.py /NAS/MIDP/DBMSVC/MIDP/TDIA
## python3 sql_lng_002.py /NAS/MIDP/DBMSVC/MIDP/TDM
## 2026.02.25 클로드
## 아래와 같이 파이썬 소스구현 요청합니다.
## 1. 2개소스 내용 설명
## 아래 "[sql_dat_file.py 소스]"
## 이 소스에서 파라미터러 받은 디렉토리의 하위소스에서
## MAIN_QUERY_START의 패턴으로 시작해서  ";"까지의 내용을
## queries에 저장했다가 .dat파일로 행단위로 저장하는데
## 
## "[sql_srctgt_csv.py 소스]" 에서는 파일로 바로생성하지 않고 queries의 내용을 ";"까지의 쿼리단위로 분석을 하여 
## 아래레이웃대로
## crud_type,abs_path,file,full_path,sql_typ,source_table,target_table 정보를 추출하여
## .csv로 저장하고 있는구조
## 
## 2. 요청목적
## 지금요청하는 것은
## 이 2개의 "[sql_dat_file.py 소스]" 와 "[sql_srctgt_csv.py 소스]" 소스에 대한 내용을 
## 하나로 합쳐서 하나의 소스로 구현하는것이 목적입니다.
## 
## 3. 소스요청상세내용
## "[sql_dat_file.py 소스]" 소스에서는 queries 추출하는 로직을 적용하고
## "[sql_srctgt_csv.py 소스]" 에서는 queries의 내용을 ";" 까지의 쿼리들에 대해서
## sql_typ에 따라 소스와 타겟테이블로 구분 분석하는 로직을 적용하여
## "[sql_srctgt_csv.py 소스]" 에서의 쿼리부분 분석하는 로직과 출력방식 등을 유지하여 
## 
## crud_type,abs_path,file,full_path,sql_typ,source_table,target_table 레이아웃대로
## .csv파일로 생성하는 소스를 sql_lng_001.py 파일명으로 구현을 요청합니다.
## 
## 아래내용 수정요청1,수정요청2,수정요청3,수정요청4 를 적용해서 소스수정요청합니다.
## sql_lng_002.py 파일로 생성
## 결과적으로 보니 MAIN_QUERY_START 리스트에서 declare와 begin을 제외하면 될것으로 보입니다.
## 수정요청1 :아래의 쿼리 경우 sql_type이 "declare" 생성되는데 "delete" 로 생성되도록 수정
## declare
## begin
## delete from ${T_A}.TB_0;
## 
## 
## 수정요청2 : 아래의 구조의 경우 ql_type이 "declare" 생성되는데 "select" 생성되도록 수정
## declare -r TRUE=1
## function chk()
## {
## select  col from tb;
## }
## 
## 수정요청3 : 아래의 경우는 sql_type이 "begin"으로 생성되는데 "delete" 생성되도록 수정
## declaer
##   v_c number :=0; 
## begin 
##  delete
...
##    where a=b;
##   
## 수정요청4 : 아래의 경우는 sql_type이 "begin"으로 생성되는데 "update" 생성되도록 수정
## declaer
##   v_c number :=0; 
## begin 
##  update ${T_A}TB_1 a
...
#!/usr/bin/env python3
# sql_lng_002.py
# 실행방법: python3 sql_lng_002.py 절대경로포함소스디렉토리
#
# [통합 내용]
# - sql_dat_file.py  : 쿼리 추출 로직 (전처리, MAIN_QUERY_START 패턴, END IF/COMMIT 제외 등)
# - sql_srctgt_csv.py: 소스/타겟 테이블 분석 로직 및 CSV 출력
...
if len(sys.argv) != 2:
    print("사용법: python3 sql_lng_002.py 절대경로포함소스디렉토리")
    sys.exit(1)
...
# 제외할 패턴 목록 (sql_dat_file.py 방식 - 리스트로 관리)
EXCLUDE_PATTERNS = [
...
#    두 소스의 합집합 - sql_dat_file.py의 넓은 범위 채택
# ==============================
...
# ── DECLARE / BEGIN 블록 처리 ───────────────────────────────────────────
# 수정요청1~4:
#   DECLARE/BEGIN 은 PL/SQL 블록 래퍼이므로 MAIN_QUERY_START 에서 제거.
#   블록 내부의 첫 번째 실질 DML 키워드를 sql_type 으로 사용한다.
#   (블록 전체가 하나의 쿼리로 넘어왔을 때 sql_type 보정용)
            ...
            for query in queries:
                ...
                for tgt in sorted(targets, key=lambda x: x or ''):
                    for src in sorted(real_sources, key=lambda x: x or ''):
                        # 실제 소스가 있는데 src=None 인 행은 출력 제외
