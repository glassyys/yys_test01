## python3 sql_lng_001.py /home/p190872/chksrc/test
## python3 sql_lng_001.py /home/p190872/chksrc/SIDHUB
## python3 sql_lng_001.py /NAS/MIDP/DBMSVC/MIDP/SID/SRC/SIDHUB
## python3 sql_lng_001.py /NAS/MIDP/DBMSVC/MIDP/SID
## python3 sql_lng_001.py /NAS/MIDP/DBMSVC/MIDP/TMT
## python3 sql_lng_001.py /NAS/MIDP/DBMSVC/MIDP/TDIA
## python3 sql_lng_001.py /NAS/MIDP/DBMSVC/MIDP/TDM
## 2026.02.25 클로드
...
#!/usr/bin/env python3
# sql_lng_001.py
# 실행방법: python3 sql_lng_001.py 절대경로포함소스디렉토리
#
# [통합 내용]
...
if len(sys.argv) != 2:
    print("사용법: python3 sql_lng_001.py 절대경로포함소스디렉토리")
    sys.exit(1)
...
def detect_real_sql_type(query):
    """쿼리의 실질 DML 타입 반환 (WITH 시작 쿼리도 처리)"""

    q = query.strip().upper()
    words = q.split()
    first_word = words[0] if words else "UNKNOWN"

    # WITH 시작이 아니면 첫 키워드 반환
    if first_word != "WITH":
        # CREATE TEMPORARY VIEW / CREATE OR REPLACE VIEW 등 → CREATE
...
def classify_sql_type(sql_typ):
    ...
    if sql_up in ("CREATE", "INSERT", "MERGE", "REPLACE", "UPSERT",
