...
def analyze_file(file_path, schema_map):
    ...
    for block in blocks:
        ...
        for part in filter(None, [cte_sql, main_sql]):
            upper = part.upper()
            for m in FROM_JOIN_PATTERN.finditer(upper):
                read_sql = part[m.start():]
        ...
        for s in sources:
            key = (s[0], s[1], target[0], target[1], file_name)
            if key in dedup:
                continue
            dedup.add(key)
