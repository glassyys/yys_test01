...
## python3 /home/p190872/chksrc/sql_tb_chk_v08_merge_db.py /NAS/MIDP/DBMSVC/MIDP/TMT
## python3 /home/p190872/chksrc/sql_tb_chk_v08_merge_db.py /NAS/MIDP/DBMSVC/MIDP/TDIA
## 
...
# OUT_DIR 정의: 스크립트가 위치한 디렉토리 아래 out/ 폴더
...
def analyze_file(file_path: str, schema_map: dict, op_dtm: str) -> list:
    results = []
...
def analyze_directory(target_dir: str, schema_map: dict, op_dtm: str) -> tuple:
    """
    Returns:
    ...
    for root, _, files in os.walk(os.path.abspath(target_dir)):
        for f in files:
            ...
            file_count += 1
            all_results.extend(analyze_file(file_path, schema_map, op_dtm))
...
# ===============================================================
# 3. TRUNCATE → INSERT INTO  /  테이블 자동 생성 포함
# ===============================================================
...
def load_to_mysql(results: list, csv_name: str, mysql_cfg: dict) -> dict:
    ...
    table_name = os.path.splitext(os.path.basename(csv_name))[0]
        ...
        with conn.cursor() as cur:
            ...
            data = [
                (
                    ...
                    r["target_schema"],  r["target_table"],  r["op_dtm"],
                )
                for r in results
            ]
...
if __name__ == "__main__":
    ...
    results, file_count = analyze_directory(target_directory, schema_map, op_dtm)
    ...
    # ── CSV 저장
    csv_path = save_csv(results, target_directory)
    print(f"  저장 CSV 파일  : {csv_path}")

    # ── MySQL TRUNCATE → INSERT
    print("\n[INFO] MySQL 적재 시작 ...")
    db_info = load_to_mysql(results, csv_path, mysql_cfg)
