...
# ★ OUT_DIR 전역 변수 정의 (스크립트 위치 기준 ./out/)
...
# ===============================================================
# CSV 저장 ★ 변경: 실행 디렉토리 하위 out/ 에 생성
# ===============================================================

def save_csv(results: list, target_dir: str) -> str:
    """
    CSV 파일을 스크립트 위치 기준 ./out/ 디렉토리에 저장한다.
    out 디렉토리가 없으면 자동 생성한다.
    """
    # out 디렉토리 생성 (이미 있으면 무시)
    ...
    csv_path = os.path.join(OUT_DIR, csv_file)   # 절대 경로 조합
    ...
    return csv_path   # ★ 절대 경로 반환 (파일명만 → 전체 경로로 변경)
...
def load_to_mysql(results: list, csv_path: str, mysql_cfg: dict) -> dict:
    """
    ...
    table_name = os.path.splitext(os.path.basename(csv_path))[0]   # CSV 파일명 (확장자 제외)
