...
#!/usr/bin/env python3
# ===============================================================
# sql_v10_full_emrput.py
#
# 실행예시:
#   python3 sql_v10_full_emrput.py <분석대상_디렉토리>
...
def parse_args():
    ...
    if src_dir is None:
        print("사용법: python3 sql_v10_full_emrput.py 소스디렉토리 "
              "[--mode SIMPLE|DETAIL] [--db] [--conf 경로]")
        sys.exit(1)
...
def main():
    ...
    for root, _, files in os.walk(SOURCE_DIR):
        for file in sorted(files):
            ...
            # ===========================
            # [수정] EMRPUT 라인 수집: file_name -> file, dir_file -> full_path
            # ===========================
            emr_lines = extract_emrput_lines(full_path)
            for line in emr_lines:
                emrput_rows.append([
                    base_directory,
                    file,       # 수정: file_name(미정의) → file(루프 변수)
                    full_path,  # 수정: dir_file(미정의) → full_path(루프 내 변수)
                    line
                ])
    ...
    # ===========================
    # [수정] EMRPUT CSV 생성: 미정의 변수를 기존 상수/변수로 교체
    # ===========================
    if emrput_rows:
        os.makedirs(OUT_DIR, exist_ok=True)
        source_last_dir = os.path.basename(os.path.normpath(SOURCE_DIR))  # 수정
        timestamp       = op_dtm.replace("-", "").replace(" ", "_").replace(":", "")  # 수정
        emr_output_path = os.path.join(
            OUT_DIR,                                                        # 수정: out_dir → OUT_DIR
            f"{PROGRAM_NAME}_{source_last_dir}_emrput_{timestamp}.csv"     # 수정: program_name → PROGRAM_NAME
        )
