...
#
# 실행예시:
#   python3 sql_find_v001.py <검색대상_디렉토리> <검색할_칼럼명> [--conf mysql.conf 경로]
#
# 예시:
#   python3 sql_find_v001.py /NAS/MIDP/DBMSVC/MIDP/SID/SRC/SIDHUB target_table
#
# 설명:
#   1. 실행 소스의 하위 디렉토리 'out' 내부의 모든 .csv 파일을 탐색하여 읽습니다.
#   2. 첫 행을 칼럼명으로 인식하여, 지정한 '검색할_칼럼명'의 값들(단어 리스트)을 추출하고 메모리에 저장합니다.
#   3. 지정된 '검색대상_디렉토리' 하위 소스 파일(.sql, .hql, .uld, .ld, .sh)을 재귀적으로 스캔합니다.
#   4. 메모리에 저장된 단어 기준으로 각 라인에 해당 단어가 포함되어 있는지 탐색합니다.
#   5. 매칭된 라인 정보, 소스 파일 정보, 디렉토리 정보를 기존 소스 규칙에 따라 생성된 MySQL 테이블에 등록합니다.
#      - 테이블명 규칙: sql_find_v001_<검색대상_디렉토리의_마지막폴더명>
# ===============================================================
...
# ============================================================
# DB 적재 (DROP -> CREATE -> BATCH INSERT)
# ============================================================
def db_insert_matches(rows_buffer: list, run_id: str, op_dtm: str,
                      mysql_conf: dict, source_dir: str) -> tuple:
...
def parse_args():
    ...
    if search_dir is None or column_name is None:
        print(f"사용법: python3 {PROGRAM_NAME}.py <검색대상_디렉토리> <검색할_칼럼명> [--conf mysql.conf 경로]")
        sys.exit(1)
    ...
    return search_dir, column_name, conf_path

# ============================================================
# MAIN
# ============================================================
def main():
    search_dir, column_name, conf_path = parse_args()
    ...
    # 1. out 디렉토리 하위 .csv 파일들을 읽어 단어 리스트 추출
    search_words = set()
    ...
    if not csv_files:
        print(f"[ERROR] 'out' 디렉토리 내에 .csv 파일이 존재하지 않습니다: {OUT_DIR}")
        sys.exit(1)
    ...
    for csv_file in sorted(csv_files):
        ...
        try:
            with open(csv_path, "r", encoding="utf-8-sig") as f:
                ...
                print(f"  - {csv_file}: '{column_name}' 컬럼에서 {count_in_file:,}개 단어 추출 완료")
        except Exception as e:
            print(f"  - [WARN] {csv_file} 파일 읽기 실패: {e}")

    if not search_words:
        print(f"[ERROR] 모든 CSV 파일 내에서 '{column_name}' 컬럼 값을 찾지 못했거나 값이 모두 비어 있습니다.")
        sys.exit(1)
    ...
    print("=" * 60)
    if db_err:
        print(f" 소스 키워드 탐색 적재 실패")
        print("=" * 60)
        print(f"  DB 오류 내용       : {db_err}")
    else:
        print(f" 소스 키워드 탐색 성공 완료")
        print("=" * 60)
